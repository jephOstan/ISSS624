...
```{r}
pacman::p_load(sf, tidyverse, questionr, janitor, psych, ggplot2, gcookbook, tmap, ggpubr, egg, corrplot, gtsummary, regclass, caret, heatmaply, ggdendro, cluster, factoextra, spdep, ClustGeo, GGally, skimr, stringr, funModeling, knitr)
```
:::
...
| 8.578262 | [11.5 8.5](https://latitude.to/map/ng/nigeria/regions/kano-state/) |             |          |             |                      |                                                                                                                                                                                                                                                                                                                           |
...
#### 3.7.3.1 plot boxplot "pct_functional"

```{r}
ggplot(data=cluster_vars, 
       aes(x=`pct_functional`)) +
  geom_boxplot(color="black", 
               fill="#543005")
```

#### 3.7.3.2 plot boxplot "pct_nonFunctional"
...
#### 3.7.3.3 plot boxplot "pct_handPump"
...
### 3.7.4 Multi-plot Histogram

::: {.callout-warning appearance="simple" icon="false"}
```{r}
pctFunctional <- ggplot(data = cluster_vars,
                         aes(x = `pct_functional`))+
  geom_histogram(bins=10,
                 colour = "black",
                 fill = "#543005")

pctNonFunctional <- ggplot(data = cluster_vars,
                         aes(x = `pct_nonFunctional`))+
  geom_histogram(bins=10,
                 colour = "black",
                 fill = "#C16622FF")
...
```{r}
ggarrange(pctFunctional,pctNonFunctional,
          ncol = 2)
```
:::
...
::: {.callout-warning appearance="simple" icon="false"}
```{r}
wp_ngaTrans1 <- wp_ngaTrim[-86,]
cluster_varsTrim1 <- cluster_varsWM[-86,]
