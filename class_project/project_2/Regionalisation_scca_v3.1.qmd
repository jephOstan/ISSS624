...
### 3.2.4 Save Imported Data Frame into RDS Format
...
```{r}
#| code-fold: false
wp_joined.sf %>% janitor::get_dupes(shapeName,
                                 lat_lon_deg)
```
...
### ?.1.2 EDA Clustering Table

Before standardisation,

#### ?.1.2.1 Helper Function 1
...
#### ?.1.2.1 Helper Function 2

```{r}
#| eval: false
hist_plot <- function(varname, title){
  func3 <- ggplot(data = cluster_varsNF1, 
       aes(x = varname)) + 
  geom_histogram(bins = 30,
                 color = "black", 
                 fill = "steelblue") +
...
#### ?.1.3.1 visualise cluster variable

```{r}
#| eval: false
hist_box_plot(wp_ngaTrim$wp_nonFunctional, "Non-Functional Water Points")
```
